...
type RelationshipCategory = 'alliance' | 'rivalry' | 'hostility' | 'loyalty' | 'family' | 'control' | 'debt' | 'custom'
type RelationshipDirection = 'directed' | 'mutual'
...
const TEXT = {
  ...
  es: {
    ...
    category: 'Tipo de conexión',
    strength: 'Intensidad',
    direction: 'Dirección',
    directed: 'Unidireccional',
    mutual: 'Mutua',
    alliance: 'Alianza',
    rivalry: 'Rivalidad',
...
function ChronicleEntityNode({ data }: NodeProps<ChronicleNode>) {
  ...
  return (
    <article
      style={data.accentColor ? { '--cc-node-accent': data.accentColor } as any : undefined}
      className={[
        'cc-rel-node',
    ...
    >
      ...
      <div className="cc-rel-node__copy">
        <div className="cc-rel-node__meta">
          <span title={data.entity.subtitle}>{data.entity.subtitle}</span>
          {data.gmOnly && <LuEyeOff aria-label="GM only" />}
        </div>
        <strong title={data.entity.name}>{data.entity.name}</strong>
      </div>
...
const RELATION_COLORS: Record<RelationshipCategory, string> = {
  alliance: '#78a77b',
  rivalry: '#c58a57',
  hostility: '#c86f6f',
  loyalty: '#6f93c8',
...
const RELATION_CATEGORIES: RelationshipCategory[] = [
  'alliance',
  'rivalry',
  'hostility',
  'loyalty',
...
function RelationshipsCanvas({
  ...
  const [relationshipCategory, setRelationshipCategory] = useState<RelationshipCategory>('custom')
  const [relationshipStrength, setRelationshipStrength] = useState(3)
  const [relationshipDirection, setRelationshipDirection] = useState<RelationshipDirection>('directed')
    ...
    try {
      ...
      const [
        ...
        supabase.from('organizations')
          .select('id,name,organization_type,image_path,visibility,color')
          .eq('campaign_id', campaignId),
        ...
        supabase.from('campaign_relationships')
          .select('id,campaign_id,source_type,source_id,target_type,target_id,relationship_type,notes,visibility,relationship_category,strength,direction,created_by')
          .eq('campaign_id', campaignId)
  ...
  useEffect(() => {
    ...
    const nextEdges: Edge[] = relationships
      ...
      .map((relationship) => {
        ...
        const category = relationship.relationship_category || 'custom'
        const relationColor = RELATION_COLORS[category]
        const strength = Math.max(1, Math.min(5, relationship.strength || 3))
        const marker = {
        ...
        return {
          ...
          animated: false,
          markerEnd: marker,
          markerStart: relationship.direction === 'mutual' ? marker : undefined,
          className: [
            'cc-rel-edge',
            `cc-rel-edge--${category}`,
            active ? 'is-active' : 'is-dimmed',
            relationship.visibility === 'gm_only' ? 'is-private' : '',
          ].filter(Boolean).join(' '),
          labelStyle: {
            fill: active ? relationColor : '#5e5d5a',
            fontSize: 11,
            fontWeight: 700,
          },
          labelBgStyle: {
          ...
          style: {
            stroke: active ? relationColor : '#3d4149',
            strokeWidth: active ? 1 + strength * 0.42 : 1,
            opacity: active ? 0.94 : 0.12,
          },
        }
      })
  ...
  const connectNodes = useCallback(
    (connection: Connection) => {
      ...
      setRelationType('')
      setRelationNotes('')
      setRelationshipCategory('custom')
      setRelationshipStrength(3)
      setRelationshipDirection('directed')
  ...
  const openEditRelationship = (relationship: RelationshipRecord) => {
    ...
    setRelationshipCategory(relationship.relationship_category || 'custom')
    setRelationshipStrength(relationship.strength || 3)
    setRelationshipDirection(relationship.direction || 'directed')
                  ...
                  return (
                    ...
                    >
                      <div className="cc-rel-connection__flow">
                        ...
                        <span>
                          {relationship.direction === 'mutual' ? '↔' : selectedIsSource ? '→' : '←'}
                          {' '}
                          {relationship.relationship_type}
                        </span>
                      </div>

                      <div className="cc-rel-connection__meta">
                        <span
                          className="cc-rel-category-badge"
                          style={{ '--relation-color': RELATION_COLORS[relationship.relationship_category || 'custom'] } as any}
                        >
                          <i />
                          {t[relationship.relationship_category || 'custom']}
                        </span>
                        <span className="cc-rel-strength-badge">{t.strength} {relationship.strength || 3}/5</span>
                        <span className="cc-rel-direction-badge">{relationship.direction === 'mutual' ? `↔ ${t.mutual}` : `→ ${t.directed}`}</span>
                      </div>
      ...
      {relationModal && (
        ...
        >
          ...
          >
            ...
            <div className="cc-rel-v2-options">
              <div className="cc-rel-v2-option">
                <span>{t.category}</span>
                <div className="cc-rel-category-grid">
                  {RELATION_CATEGORIES.map((category) => (
                    <button
                      key={category}
                      type="button"
                      className={relationshipCategory === category ? 'is-active' : ''}
                      style={{ '--relation-color': RELATION_COLORS[category] } as any}
                      onClick={() => {
                        setRelationshipCategory(category)
                        if (!relationType.trim() && category !== 'custom') {
                          setRelationType(t[category])
                        }
                      }}
                    >
                      <span className="cc-rel-category-dot" />
                      {t[category]}
                    </button>
                  ))}
                </div>
              ...
              <div className="cc-rel-v2-option">
                ...
                <div className="cc-rel-strength-picker">
                  {[1, 2, 3, 4, 5].map((value) => (
                    <button
                      key={value}
                      type="button"
                      className={relationshipStrength === value ? 'is-active' : ''}
                      onClick={() => setRelationshipStrength(value)}
                      aria-label={`${t.strength} ${value}`}
                    >
                      {value}
                    </button>
                  ))}
                  <small>{relationshipStrength === 1 ? t.weak : relationshipStrength === 5 ? t.defining : `${relationshipStrength}/5`}</small>
                </div>
              </div>

              <div className="cc-rel-v2-option">
                <span>{t.direction}</span>
                <div className="cc-rel-direction-picker">
                  <button type="button" className={relationshipDirection === 'directed' ? 'is-active' : ''} onClick={() => setRelationshipDirection('directed')}>→ {t.directed}</button>
                  <button type="button" className={relationshipDirection === 'mutual' ? 'is-active' : ''} onClick={() => setRelationshipDirection('mutual')}>↔ {t.mutual}</button>
                </div>
              </div>
            </div>
